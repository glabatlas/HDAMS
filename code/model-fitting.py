...
def leave_one_out(fold_num, X_matrix, y_target, *classifiers):
    ...
    for train_index, test_index in kf.split(X_matrix, y_target):
        ...
        if len(classifiers)==1:
            ...
            score_i=0
            y_pred_prob = np.zeros_like(y_test)
            for classifier in classifiers:
                classifier.fit(X_train, y_train)
                score_i = score_i + classifier.score(X_test, y_test)
                y_pred_prob = y_pred_prob + classifier.predict_proba(X_test)[:, 1]
       ...
       #####  Micro-strategy based ROC ###
        dic_fpr["micro"], dic_tpr["micro"], tholds_roc = roc_curve(y_test, y_pred_prob)
        dic_tpr["micro"] = np.interp(mean_fpr, dic_fpr["micro"], dic_tpr["micro"])
        dic_auc["micro"] = auc(mean_fpr, dic_tpr["micro"])

        dic_precision["micro"], dic_recall["micro"], tholds_prc = precision_recall_curve(y_test, y_pred_prob)
...
def run_classifier_KFold(k, X_train, y_target):
    ...
    auprc = ls_prc_res[2]
    plt.plot(ls_fpr, ls_tpr, 'b')

    ftemp.writelines("fpr\ttpr\tSVM\n")
    ...
    # print (len(X_train), len(y_target))
    ...
    ls_tpr = ls_roc_res[1]
    auroc = ls_roc_res[2]
    plt.plot(ls_fpr, ls_tpr, 'g')

    ls_recall = ls_prc_res[0]
    ls_precision = ls_prc_res[1]
    auprc = ls_prc_res[2]

    ftemp.writelines("fpr\ttpr\tLR\n")
    for fpr, tpr in zip(ls_fpr, ls_tpr):
        ftemp.writelines('%.3f\t%.3f\n' % (fpr, tpr))
...
if __name__ =="__main__":
    ...
    fitting_model = classifier(selected_data, data_lable, 2)
    path = "./model/"
